...
export const mw_weapons_firing_phase_inner: Middleware = ({
	getState,
	dispatch,
}) => () => () => {
	...
	ship_ids.forEach((id) => {
		...
		oc(bogey)
			.weaponry.firecons([])
			.forEach((firecon, firecon_id) => {
				if (!firecon.target_id) return;
				debug(firecon.id);

				(_.filter(oc(bogey).weaponry.weapons([]), {
					firecon_id,
				}) as any).forEach(({ id: weapon_id }: WeaponState) => {
					dispatch(
						fire_weapon(bogey.id, firecon.target_id, weapon_id),
					);
				});
			});
	});
