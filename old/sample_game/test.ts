...
const flattenLog = (log: any): any => {
	return _.flatten(
		log.map((e: any) =>
			e.subactions ? [e, ...flattenLog(e.subactions)] : [e],
		),
	).map((e: any) => _.omit(e, 'subactions'));
};
...
const filterLogAction = _.curry(
	(action: ActionCreator, log: LogState | LogAction = []): LogState => {
		...
		return log.reduce((accum: LogState, logEntry: LogAction) => {
			...
			return [
				...accum,
				...filterLogAction(action, _.get(logEntry, 'subactions', [])),
			];
		}, []);
	},
);
...
turns[3] = {
	actions: [
		...['enkidu', 'siduri'].map((bogey_id) =>
			set_orders(bogey_id, { navigation: { thrust: -1 } }),
		),
		play_turn(),
	],
	dice: [[4, 1], []],
	tests(state) {
		...
		[enkidu, siduri].forEach((ship) =>
			expect(ship).toHaveProperty('navigation.velocity', 0),
		);
...
const manage_turn = function (battle: any, directives: TurnDirective) {
	...
	_.get(directives, 'dice', []).forEach((result: any) =>
		dice.default.mockImplementationOnce(() => result),
	);
...
describe.each(turns.map((t, i) => [i, t]))(
	'turns',
	function (i: any, turn: any) {
