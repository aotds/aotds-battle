...
export default class Redactor<A extends Action = Action, S = {}> {
    ...
    public for<A extends ActionGroup>(actions: A, reaction: UpReducer<S, ActionFor<A[keyof A]>>): void;
    public for(action: CatchAll, reaction: UpReducer<S, A>): UpReducer<S, A>;
    public for<B extends A>(action: (...args: any[]) => B, reaction: UpReducer<S, B>): UpReducer<S, A>;
    public for(action: any, reaction: any) {
        if (typeof action === 'function' || action === '*') return this.addRedaction(action, reaction);
    ...
    public reduce(state: S | undefined, action: A) {
        ...
        let type = this.redactions[action.type] ? action.type : this.redactions['*'] ? '*' : null;

        if (this.debug) {
            this.debug('reducing action %s with redactor %s', action.type, type || 'noop');
        }
