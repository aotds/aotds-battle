...
type InternalCheck = (
	bogey: BogeyState,
) => undefined | InternalSystem | InternalSystem[];
...
export default mw_for(
	damage,
	({ getState, dispatch }) => (next) => (
		...
		if (delta > 0) {
			const threshold =
				(100 * delta) /
				(fp.getOr(1)(['structure', 'hull', 'rating'])(
					bogey as any,
				) as number);
			...
			_.flattenDeep(
				...
				.map((system) => ({
					...
					check: {
						...
						die: dice(1, {
							nbr_faces: 100,
							note: `internal damage check ${JSON.stringify(
								system,
							)}`,
						})[0],
					},
				...
				.map((id) => ({
					hit: id.check.die <= id.check.threshold,
					...id,
				}))
				.filter(({ hit }) => hit)
				.map((id) => internal_damage(bogey_id, id))
				.map((x) => dispatch(x));
		}
	},
);
