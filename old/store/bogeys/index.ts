...
updux.addMutation(init_game, ({ bogeys }) => () => fp.keyBy('id', bogeys));
updux.addMutation(play_turn, () => fp.omitBy('structure.destroyed'));

function reduce_single_bogey(prop = 'id') {
	return (payload: any, action: Action) =>
		u.updateIn(
			payload[prop],
			u.if((x: unknown) => x, bogey_upreducer(action)),
		);
}
