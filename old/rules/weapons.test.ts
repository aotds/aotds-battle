...
test('basic', () => {
	let attacker = {
		navigation: { coords: [0, 0], heading: 0, velocity: 0 },
	} as BogeyState;
	let target = {
		navigation: { coords: [10, 1], heading: 6, velocity: 0 },
	} as BogeyState;
	let weapon = {};
	...
	let result = fire_weapon(attacker, target, {
		weapon_type: 'beam',
		weapon_class: 1,
		arcs: ['FS'],
	} as WeaponState);
...
describe('target bearing', () => {
	...
	fp.times(Number)(12).forEach((heading) =>
		test(`heading ${heading}`, () => {
			...
			let type = [0, 1, 11].some((x) => heading === x)
...
describe('aft', () => {
	let attacker = {
		navigation: { coords: [0, 0], heading: 6 },
		drive: {},
	} as any;
	let target = { navigation: { coords: [0, 10], heading: 6 } } as any;
	...
	test('thrust used? no fire', () => {
		let result = fire_weapon(
			u.updateIn('navigation.thrust_used', 2)(attacker),
			target,
			weapon,
		);
