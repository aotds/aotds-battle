...
describe("internal damage", () => {
  test("drive", () => {
    let bogey = bogey_reducer(
      sample_bogey as any,
      internal_damage("enkidu", "drive", true)
    );
    ...
    // boom! again
    bogey = bogey_reducer(bogey, internal_damage("enkidu", "drive", true));
  ...
  test("firecons", () => {
    let bogey = bogey_reducer(
      sample_bogey as any,
      internal_damage("enkidu", ["firecon", 1], true)
    );
  ...
  test("weapons", () => {
    let bogey = bogey_reducer(
      sample_bogey as any,
      internal_damage("enkidu", ["weapon", 1], true)
    );
