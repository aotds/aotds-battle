...
const redactor = new Redactor([] as FireconState[],undefined,'aotds:reducer:bogeys:bogey:weaponry:firecons');
...
redactor.addRedaction(
    ...
    (action) => u({
        [ action.payload.firecon_id ]: u.if( _.identity, firecon_upreducer(action) )
    })
);
