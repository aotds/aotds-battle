...
updux.addMutation(internal_damage, ({ system, hit }) => drive => {
    ...
    drive = u({ damage_level: fp.flow([fp.defaultTo(0), fp.add(1), fp.clamp(0, 2)]) }, drive);

    // if we are here it's a least a level of 1
    drive = uu(({ damage_level, rating }) => u({ current: damage_level === 2 ? 0 : Math.floor(rating / 2) }))(drive);
