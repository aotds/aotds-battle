...
test('inflate', () => {
    expect(
        ...
        shields: [{ id: 0, level: 1 }, { id: 1, level: 1 }, { id: 2, level: 2 }],
    });
});
...
test('shields', () => {
    ...
    struct = structure_reducer(struct, internal_damage('enkidu', ['shield', 1], true));
...
describe('damage', () => {
    ...
    tests.forEach(({ action, hull, armor }) =>
        test(JSON.stringify(action), () =>
            expect(
                structure_reducer(
                    { hull: { current: 10 }, armor: { current: 2 } } as StructureState,
                    {
                        type: 'DAMAGE',
                        payload: action,
                    } as Action,
