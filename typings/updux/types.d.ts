...
export declare type MutationEntry = [ActionCreator | string, Mutation<any, Action<string, any>>, boolean?];
export declare type GenericActions = Dictionary<ActionCreator<string, (...args: any) => {
    type: string;
}>>;
export declare type UnionToIntersection<U> = (U extends any ? (k: U) => void : never) extends (k: infer I) => void ? I : never;
export declare type StateOf<D> = D extends {
    initial: infer I;
} ? I : unknown;
export declare type DuxStateCoduxes<C> = C extends Array<infer U> ? UnionToIntersection<StateOf<U>> : unknown;
export declare type DuxStateSubduxes<C> = C extends {
    '*': infer I;
} ? {
...
declare type DuxStateGlobSub<S> = S extends {
    '*': infer I;
} ? StateOf<I> : unknown;
/** @ignore */
declare type AggDuxState2<L, S, C> = (L extends never[] ? Array<DuxStateGlobSub<S>> : L & DuxStateSubduxes<S>) & DuxStateCoduxes<C>;
/** @ignore */
export declare type AggDuxState<O, S extends UpduxConfig> = unknown extends O ? AggDuxState2<S['initial'], S['subduxes'], S['coduxes']> : O;
declare type SelectorsOf<C> = C extends {
    selectors: infer S;
} ? S : unknown;
/** @ignore */
export declare type DuxSelectorsSubduxes<C> = C extends object ? UnionToIntersection<SelectorsOf<C[keyof C]>> : unknown;
/** @ignore */
export declare type DuxSelectorsCoduxes<C> = C extends Array<infer U> ? UnionToIntersection<SelectorsOf<U>> : unknown;
declare type MaybeReturnType<X> = X extends (...args: any) => any ? ReturnType<X> : unknown;
declare type RebaseSelector<S, X> = {
    [K in keyof X]: (state: S) => MaybeReturnType<X[K]>;
};
declare type ActionsOf<C> = C extends {
    actions: infer A;
} ? A : {};
export declare type DuxActionsCoduxes<C> = C extends Array<infer I> ? UnionToIntersection<ActionsOf<I>> : {};
declare type ItemsOf<C> = C extends object ? C[keyof C] : unknown;
export declare type DuxActions<A, C extends UpduxConfig> = A extends object ? A : UnionToIntersection<ActionsOf<C | ItemsOf<C['subduxes']> | ItemsOf<C['coduxes']>>>;
export declare type DuxSelectors<S, X, C extends UpduxConfig> = unknown extends X ? RebaseSelector<S, C['selectors'] & DuxSelectorsCoduxes<C['coduxes']> & DuxSelectorsSubduxes<C['subduxes']>> : X;
export declare type Dux<S = unknown, A = unknown, X = unknown, C = unknown> = {
 ...
 * ```
 */
export declare type UpduxConfig = Partial<{
    initial: unknown /** foo */;
    subduxes: Dictionary<Dux>;
    coduxes: Dux[];
...
export declare type UpduxMiddleware<S = any, X = Dictionary<Selector>, A = Action> = (api: UpduxMiddlewareAPI<S, X>) => (next: Function) => (action: A) => any;
export declare type Selector<S = any> = (state: S) => unknown;
export declare type DuxState<D> = D extends {
    initial: infer S;
} ? S : unknown;
export {};
