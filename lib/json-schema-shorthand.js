...
function sh_json_schema(obj) {
  ...
  ['definitions'].filter(k => obj.hasOwnProperty(k)).forEach(keyword => {
    obj[keyword] = _.mapValues(obj[keyword], v => sh_json_schema(v));
  });
  ['anyOf', 'allOf', 'oneOf'].filter(k => obj.hasOwnProperty(k)).forEach(keyword => {
    obj[keyword] = obj[keyword].map(v => sh_json_schema(v));
  });
  ['not'].filter(k => obj.hasOwnProperty(k)).forEach(keyword => {
  ...
  if (obj.hasOwnProperty('properties')) {
    obj.properties = _.mapValues(obj.properties, v => sh_json_schema(v));
  }

  if (obj.hasOwnProperty('properties')) {
    let required = obj.required || [];
    required = required.concat(_.keys(obj.properties).filter(k => obj.properties[k].required));
    obj.properties = _.mapValues(obj.properties, v => _.omit(v, 'required'));
