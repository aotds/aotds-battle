...
function ValidateSchema(options) {
    ...
    let validate = function (schema) {
        ...
        shall_validate.call(data, schema).catch(errors => {
            StackTrace.get().then(stack => {
                options.on_validator_error('', { value: data, errors,
    ...
    let with_args = function (args_type) {
        let schema = typeof args_type == 'string' ? { '$ref': 'https://localhost/validate-schema/' + args_type } : _extends({ type: 'array' }, args_type);
    ...
    let returns = function (return_type) {
        let validate = validator.compile(typeof return_type == 'string' ? { '$ref': 'http://localhost/validate-schema' + return_type } : return_type);
