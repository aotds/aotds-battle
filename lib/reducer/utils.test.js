...
test('combine_reducers', () => {
    ...
    let bigr = (0, _utils.combine_reducers)({ r1, r2 });
...
test('map_reducer', () => {
    let item_reducer = (state, action) => (0, _updeep2.default)({ sum: s => s + 1 })(state);

    let state = [{ id: 0, sum: 0 }, { id: 1, sum: 0 }, { id: 2, sum: 0 }];

    let mapped = (0, _utils.mapping_reducer)(item_reducer);
    let match_id = mapped(({ id }) => _lodash2.default.matches({ id }));

    let main_reducer = (0, _utils.actions_reducer)({
        DO_IT: mapped(a => _lodash2.default.matches({ id: a.id })),
        DO_IT_TOO: match_id,
    ...
    expect(state).toEqual(expect.arrayContaining([{ id: 0, sum: 0 }, { id: 1, sum: 0 }, { id: 2, sum: 0 }]));

    state = main_reducer(state, { type: 'DO_IT', id: 1 });

    expect(state).toEqual(expect.arrayContaining([{ id: 0, sum: 0 }, { id: 1, sum: 1 }, { id: 2, sum: 0 }]));

    state = main_reducer(state, { type: 'DO_IT', id: 0 });

    expect(state).toEqual(expect.arrayContaining([{ id: 0, sum: 1 }, { id: 1, sum: 1 }, { id: 2, sum: 0 }]));

    state = main_reducer(state, { type: 'DO_IT_TOO', id: 2 });

    expect(state).toEqual(expect.arrayContaining([{ id: 2, sum: 1 }]));

    state = main_reducer(state, { type: 'DO_IT_ALL' });

    expect(state).toEqual(expect.arrayContaining([{ id: 0, sum: 2 }, { id: 1, sum: 2 }, { id: 2, sum: 2 }]));
});