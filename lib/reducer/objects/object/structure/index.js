...
function inflate(state) {
    ...
    return (0, _updeep2.default)({
        hull: inflate_hull,
        armor: inflate_hull,
        shields: _updeep2.default.map(_updeep2.default.if(s => typeof s === 'number', s => ({ id: i++, level: s })))
    })(state);
}

let redact = exports.redact = {};

redact[_actions2.default.DAMAGE] = ({ damage, penetrating }) => state => {
    if (!damage) return state;

    let update;
    if (penetrating) {
        update = (0, _updeep2.default)({ hull: { current: c => c - damage } });
    } else {
    ...
    let destroy = _updeep2.default.if(s => _fp2.default.getOr(1)('hull.current')(s) <= 0, { status: 'destroyed' });
...
const reducer = (0, _utils.actions_reducer)(redact, { hull: 0, armor: 0, status: 'nominal' });
