...
const find_object = (state, id) => _lodash2.default.find(state.objects, {
  id
});

const find_weapon = (ship, id) => _lodash2.default.find(ship.weapons, {
  id
});
...
const check_if_object_destroyed = function* (action) {
  ...
  if (ship.hull <= 0) {
    yield (0, _effects.put)(_Actions2.default.destroyed({
      object_id: ship.id
    }));
  }
};
